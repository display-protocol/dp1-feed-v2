...
Claude Code reads `CLAUDE.md`, which carries the same contract and imports the same rules. Keep the two files in sync.
...
## Per-tool agent assets

Each tool wires the same reviewer and planner roles its own way. Changing one role's contract means changing all of them.
