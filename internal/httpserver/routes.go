...
// Route registration: /health, /api/v1/* ; POST and document PUT use SignatureOrAPIKeyAuth (API key or body signatures); PATCH, DELETE, and registry PUT use APIKeyAuth only. Channel routes register only when extensions are enabled.
...
// POST and document PUT use SignatureOrAPIKeyAuth (API key or signatures in body). PATCH is API-key only:
// a partial update is merged server-side, so no client signature can cover the resulting document.
// DELETE and registry PUT use APIKeyAuth only.
func RegisterRoutes(r *gin.Engine, h *Handler, cfg *config.Config, log *zap.Logger) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		if cfg.Extensions.Enabled {
			...
			v1.PATCH("/channels/:id", channelMutationDeadline, APIKeyAuth(cfg.Auth.APIKey, log), h.UpdateChannel)
