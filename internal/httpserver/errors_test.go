...
func TestMapExecutorError_signatureAuthzErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"owner_removed", executor.ErrOwnerRemoved, http.StatusForbidden, "forbidden"},
		{"owner_removed_wrapped", fmt.Errorf("x: %w", executor.ErrOwnerRemoved), http.StatusForbidden, "forbidden"},
		{"owner_consent_required", executor.ErrOwnerConsentRequired, http.StatusForbidden, "forbidden"},
		...
		{"too_many_references", executor.ErrTooManyReferences, http.StatusBadRequest, "bad_request"},
		{"resolved_too_large", executor.ErrResolvedTooLarge, http.StatusBadRequest, "bad_request"},
		{"uri_too_long", executor.ErrPlaylistURITooLong, http.StatusBadRequest, "bad_request"},
		{"no_playlist_references", executor.ErrNoPlaylistReferences, http.StatusBadRequest, "bad_request"},
...
func TestMapStoreError_alreadyExists(t *testing.T) {
	t.Parallel()
	wrapped := fmt.Errorf("create playlist: %w", &store.ConflictError{Kind: store.ErrAlreadyExists, Detail: "a playlist with this id already exists"})
