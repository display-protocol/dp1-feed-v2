...
func RequireSignatures(log *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			// A body larger than the configured cap (http.MaxBytesHandler, installed in New ahead of gin)
			// surfaces here as a MaxBytesError before we buffer it all; report it as 413 rather than a
