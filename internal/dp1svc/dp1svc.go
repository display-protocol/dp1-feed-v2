...
// verbatim, whereas `any` would re-type numbers through float64 and silently corrupt integers above
// 2^53. The encoder disables HTML escaping for the same reason (an escape is JCS-neutral, but verbatim
// is verbatim). A legacy "signature" is deliberately kept: DP-1 §7.1.3 has players prefer "signatures"
