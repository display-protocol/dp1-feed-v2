...
- **Logging:** Zap (`internal/logger`) always writes locally to stdout. When a Cloudflare Pipeline Stream URL and producer API key are configured, the same accepted entries are also converted to the immutable `service_logs_stream` schema and sent as bounded asynchronous JSON batches. Required remote dimensions are RFC 3339 timestamp, normalized level, stable service, environment, and message. Recognized tracing fields occupy their schema columns; other Zap fields stay under `structured`; `zap.Error` and direct panic/fatal entries are encoded under `exception`. The memory-only remote path caps queued records at 1,024 and 8 MiB total, rejects any record that cannot fit the 4 MiB batch bound before enqueue, and keeps at most one 4 MiB batch plus one in-flight request outside the queue. Delivery failure, queue overflow, or process crash can lose the remote copy but never suppress stdout. Graceful HTTP shutdown finishes draining handlers before logger shutdown gives every accepted default-sized batch one request-timeout window, plus scheduling margin (105 seconds total, conservatively covering combined count/byte fragmentation). Deployments should allow at least 120 seconds for the 15-second HTTP drain followed by this logger drain. No redaction is performed at this internal-service boundary.
