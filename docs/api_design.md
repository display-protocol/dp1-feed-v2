...
- **`inlineManifest`** — optional complete Ref Manifest carried on a playlist item (same level as `source`) instead of behind `ref`, per §3.6. Under playlist extension validation it is checked against the unmodified ref-manifest schema, so a malformed manifest fails the whole write with **`400`**. With extensions **off**, the core schema does not describe the field and core DP-1 tolerates unknown ones, so the manifest is stored and returned **unchecked** (same posture as `displayAt`). The feed preserves every member of the manifest, including present-but-empty ones, but does **not** guarantee byte identity: signing and JSONB storage normalize object key order, whitespace, and number spelling. JCS re-sorts keys before hashing, so signatures are unaffected — but do not byte-hash or cache the returned manifest blob. The feed does not fetch `ref`, merge the two, or apply the §3.6 precedence (a fetched `ref` manifest wins, the inline copy is the offline fallback) — that resolution belongs to players. The manifest is part of the signed payload with no `refHash` counterpart, so changing it invalidates every signature on the playlist.
