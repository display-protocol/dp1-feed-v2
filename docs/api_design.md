...
- **`schedule.byDisplayAt`** — optional playlist-level scheduling control. When true, display control layers filter items by item-level `displayAt`; this feed stores and returns the flag but does not compute playback eligibility.
- **`displayAt`** — optional ISO 8601 datetime on a playlist item (same level as `source`, not inside `display`). Accepted wire forms per §3.5.2: local datetime with seconds and no timezone (`2026-07-21T00:00:00`, display-locale local), or absolute RFC 3339 date-time with `Z`/colon offset. Date-only (`YYYY-MM-DD`) and compact offset without colon are **not** accepted. This feed stores and returns the item metadata; it does not compute playback eligibility.
